...
updateCourse(courseId: string, formData: any): void {
 ...
 /* submitForm(courseData: any) {
    ...
    // Récupérez les données du formulaire à soumettre
    const formData = this.courseForm.value;
  
    // Combiner les données des deux sections
    const combinedData = {
      ...formData, // données de la première section
      sectionData: this.sectionData // données de la deuxième section
    };
    ...
    // Appelez la méthode createCourse avec les données combinées
    this.courseService.createCourse(combinedData).subscribe(
      response => {
        console.log('Course created successfully:', response);
